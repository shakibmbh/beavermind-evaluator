...
6. Where a dimension is explicitly optional (see below), only set disabled: true if the transcript clearly meets the stated disable condition. Otherwise score it normally and set disabled: false.
7. "quickFix" describes, in one or two sentences, the single concrete thing the coach would have needed to do differently DURING THIS CALL to reach full marks on that dimension specifically.
...
- "oneThing": the single highest-leverage change to this specific call -- the one thing that would move the total score the most if fixed. "change" must be written as ONE sentence, phrased as a direct imperative instruction to the coach (command form, e.g. "Build belief by connecting every current block to the client's named long-term goal and getting them to articulate why the outcome matters" -- not "The coach should have..." or "You should..."). Start with an action verb. Reference the specific, concrete detail from this call that makes the instruction actionable (a name, a goal, a moment) rather than a generic instruction that could apply to any call. Do not quote transcript text or cite line numbers. "projectedScore" is your best estimate of the total /100 score if that one change were made, holding everything else constant.
- "brief": a tight 2-sentence evaluative assessment, roughly 45-60 words total, for whoever is reviewing this coach's performance (a manager or ops reviewer), NOT addressed to the coach directly. Written in the third person -- refer to the coach by name or as "the coach" (e.g. "Mark was technically attentive..." or "The coach was technically attentive..."), never "you" or "your". Do not quote transcript text or cite line numbers here -- describe in your own words, in clean prose a reader could follow with no knowledge of line numbering.
  Structure it as two moves, not a flat summary:
  1. Name one real, specific strength from the call, then pivot with "but"/"however" into a sharp synthesis of the main gap -- a compressed insight capturing the pattern (e.g. "coached the exercises more effectively than he coached the journey"), not a generic weakness ("could improve emotional connection").
  2. Name the concrete outcome of the call (what the client left with), then pivot with "yet"/"but" into the specific things that were missing, using language tied to the actual rubric dimensions where relevant (name the missing vision connection, accountability structure, or follow-up specificity rather than vague coaching vocabulary) -- and close on why it matters for retention or the client relationship.
  Every claim in the brief must be grounded in what actually happened in this specific call -- do not use this structure as a template to force generic praise or generic criticism if the call doesn't support it.
